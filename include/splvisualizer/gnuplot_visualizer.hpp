...
namespace splvisualizer
{
    ...
    class GnuplotDimensionalVisualizer : public Visualizer
    {
        ...
        public:
            ...
            void visualize(const spl::Log& log_planning, const spl::Log& log_execution) override
            {
                ...
                for (size_t i = 0; i < num_dims; ++i)
                {
                    ...
                    for (const spl::LogEntry& log_entry : log_planning)
                    {
                        fprintf(
                            this->gnuplot_pipe,
                            "%f %f\n",
                            log_entry.first,
                            log_entry.second.pos(i)
                        );
                    }
                    fprintf(this->gnuplot_pipe, "e\n");
                    for (const spl::LogEntry& log_entry : log_execution)
                    {
                        fprintf(
                            this->gnuplot_pipe,
                            "%f %f\n",
    ...
    class GnuplotManipulatorVisualizer : public Visualizer
    {
        ...
        public:
            ...
            void visualize(const spl::Log& log_planning, const spl::Log& log_execution) override
            {
                ...
                if (num_dims == 2)
                {
                    fprintf(this->gnuplot_pipe, "plot '-' with lines lw 2 lc rgb 'green' title 'Planned Trajectory', \\\n");
                    fprintf(this->gnuplot_pipe, "     '-' with lines lw 2 lc rgb 'red' title 'Real Trajectory'\n");
                    for (const spl::LogEntry& log_entry : log_planning)
                    {
                        fprintf(this->gnuplot_pipe, "%f %f\n", log_entry.second.pos(0), log_entry.second.pos(1));
                    }
                    fprintf(this->gnuplot_pipe, "e\n");
                    for (const spl::LogEntry& log_entry : log_execution)
                    {
                ...
                } else if (num_dims == 3)
                {
                    ...
                    fprintf(this->gnuplot_pipe, "splot '-' with lines lw 2 lc rgb 'green' title 'Planned Trajectory', \\\n");
                    fprintf(this->gnuplot_pipe, "      '-' with lines lw 2 lc rgb 'red' title 'Real Trajectory'\n");
                    for (const spl::LogEntry& log_entry : log_planning)
                    {
                        fprintf(
                            this->gnuplot_pipe,
                            "%f %f %f\n",
                            log_entry.second.pos(0),
                            log_entry.second.pos(1),
                            log_entry.second.pos(2)
                        );
                    }
                    fprintf(this->gnuplot_pipe, "e\n");
                    for (const spl::LogEntry& log_entry : log_execution)
            ...
            void visualize(const std::vector<spl::VectorRepresentation>& joint_positions)
            {
                fprintf(this->gnuplot_pipe, "set title 'Trajectory Visualizer' font ',12'\n");
                fprintf(this->gnuplot_pipe, "set xlabel 'X'\n");
                fprintf(this->gnuplot_pipe, "set ylabel 'Y'\n");
            ...
            void visualize(
                const spl::Log& log_planning,
                const spl::Log& log_execution,
                const std::vector<spl::VectorRepresentation>& joint_positions
            )
            {
                fprintf(this->gnuplot_pipe, "set title 'Trajectory Visualizer' font ',12'\n");
                fprintf(this->gnuplot_pipe, "set xlabel 'X'\n");
